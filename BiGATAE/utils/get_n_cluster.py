...
def get_N_clusters_Z(adata, n_cluster, cluster_method='louvain', range_min=0, range_max=3, max_steps=30, tolerance=0):
     """
    Tune the resolution parameter in clustering to make the number of clusters and the specified number as close as possible.
     ...
     while this_step < max_steps:
         this_resolution = this_min + ((this_max-this_min)/2)
         if cluster_method=='leiden':
             sc.tl.leiden(adata, resolution=this_resolution, key_added='BiGATAE_leiden')
         if cluster_method=='louvain':
             sc.tl.louvain(adata, resolution=this_resolution, key_added='BiGATAE_louvain')
         this_clusters = adata.obs[cluster_method].nunique()
